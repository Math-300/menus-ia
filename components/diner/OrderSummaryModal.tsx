...
import { Trash2, Receipt, ArrowRight, CheckCircle2, Plus, Minus } from 'lucide-react';
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title=" " variant="default">
      <div className="px-6 pb-6">
        ...
        <button 
            onClick={handlePay}
            disabled={selection.length === 0 || isProcessing || isSuccess}
            className={`
                w-full py-4 rounded-xl font-bold text-xs tracking-[0.15em] uppercase shadow-xl transition-all relative overflow-hidden
                ${isSuccess ? 'bg-green-500 text-white' : 'bg-brand-primary text-white hover:bg-brand-primary/90'}
        ...
        >
            {isProcessing ? (
                ...
                <div className="flex items-center justify-center gap-2">
                    <CheckCircle2 size={18} /> Orden Confirmada
                </div>
            ) : (
                <div className="flex items-center justify-center gap-2">
