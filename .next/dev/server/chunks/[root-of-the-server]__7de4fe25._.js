...
"[project]/src/app/api/admin/tenant/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET() {
    try {
        ...
        const { data: userData, error: userError } = await supabase.from('users').select('tenant_id, role').eq('id', user.id).single();
        ...
        const { data: tenant, error: tenantError } = await supabase.from('tenants').select('*').eq('id', userData.tenant_id).single();
        if (tenantError) {
...
async function PUT(request) {
    try {
        ...
        if (body.slug) {
            const { data: existingSlug } = await supabase.from('tenants').select('id').eq('slug', body.slug).neq('id', userData.tenant_id).single();
            if (existingSlug) {
                return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
        ...
        // Update tenant
        const { data: tenant, error: updateError } = await supabase.from('tenants').update(updateData).eq('id', userData.tenant_id).select().single();
        if (updateError) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__7de4fe25._.js.map