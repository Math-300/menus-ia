...
async function GET() {
    try {
        ...
        const { data: categories, error: categoriesError } = await supabase.from('categories').select('*').eq('tenant_id', userData.tenant_id).order('display_order', {
            ascending: true
        });
        if (categoriesError) {
...
async function POST(request) {
    try {
        ...
        // Create category
        const { data: category, error: createError } = await supabase.from('categories').insert({
...
//# sourceMappingURL=%5Broot-of-the-server%5D__90fb71eb._.js.map